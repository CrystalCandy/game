...
module GameConfig {
    ...
    function initPositionDefine(data) {
        ...
        //	let deepV, deepH = 3, 4
        let row = 80                //同排的距离
        let raw = FIGHT_MAP_ANGLE
        let [rowX, rowY] = [row * Math.cos(Math.abs(raw)), row * Math.sin(Math.abs(raw))]
        let cRow = 80               //左右-敌我距离
        let [cRowX, cRowY] = [cRow * Math.cos(Math.abs(raw)), cRow * Math.sin(Math.abs(raw))]
        let pRow = 60               //同向两排间的距离
        let [pRowX, pRowY] = [pRow * Math.cos(Math.abs(raw / 3)), pRow * Math.sin(Math.abs(raw / 3))]
        let list:any = {}
        
        function genPosHandler(r, cenX, cenY, index, side){
            for(let i = 1; i <= 4;i++){
                let [cX, cY] = [cenX + (i -1) * pRowX * r + r * cRowX, cenY + (i - 1) * pRowX * r + r * cRowY]
                index = index + 1
...
/*
    ...
    //==========================================寻宝系统=================================================
    export var XunBaoConfig = {};

    export function initXunBaoSystemCsv(workQueue: WorkQueue) {
        workQueue.addWorkUnit(ResWorkUnit.newObj("data\\config\\Item\\shopCommodity.csv", readCSV, this, XunBaoConfig));
    }
