...
class ClubPeopleInfoFrame extends BaseWnd {
	...
	refreshFrame() {
		...
		for (let i = 0; i < size_t(list); i++) {
			let v = list[i]
			let window = this.scroll.getItemWindow(i, group.width - 10, 130, 0, 0, 0)
		...
		let contributeText = Localize_cns("CLUB_TXT49") + "#lime" + contributeNum
	...
	initItemWindow(window) {
		...
		let Info: any = [
			...
			{ ["index_type"]: gui.Grid9Image, ["name"]: name + "_icon", ["parent"]: name + "_icon_bg", ["image"]: "zctx_90001", ["x"]: 0, ["y"]: -10, ["w"]: 140, ["h"]: 140, ["messageFlag"]: true },

			//名字and帮贡and战力
			{ ["index_type"]: gui.RichDisplayer, ["name"]: name + "_name_rd", ["parent"]: name + "_bg", ["x"]: 145, ["y"]: 20, ["w"]: 250, ["h"]: 90, ["event_name"]: null, ["fun_index"]: null, },
