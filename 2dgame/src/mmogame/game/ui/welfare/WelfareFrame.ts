class WelfareFrame extends BaseWnd {
	...
	onRefresh(){
        ...
        for (let i = 0; i < size_t(this.list); i++) {
            let v = this.list[i]
            let window = this.btn_scroll.getItemWindow(v.index, 95, 117, 0, 0, 0)
	...
	initItemWindow(window,data){
		...
		let Info: any = [
               //背景
				...
				{ ["index_type"]: eui.RadioButton, ["name"]: name +"group", ["image"]:imageName, ["font"]: "ht_20_cc_stroke",["image_down"]:imageDownName, ["x"]: 0, ["y"]: 0, ["w"]: width, ["h"]: height, ["event_name"]: null, ["fun_index"]: null},
			    ]	
