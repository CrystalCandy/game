...
class RoleSkillsSettingFrame extends BaseWnd {
	...
	public onHide(): void {
     ...
     onRefresh(){
        ...
        for(let k = 1; k <= count; k++){
			...
			let window = scroll.getItemWindow(list[k-1], 500, 154 , 0, 0)
     ...
     initItemWindow(window) {
        ...
        let mElemInfo: any = [
         { ["index_type"]: gui.Grid9Image,["name"] : name +"_bg",  ["image"]: "ty_uiDi03", ["autoScale"] :true, ["x"] : 0, ["y"] : 0,["w"] :500 ,["h"] : 154},	
		 { ["index_type"]: eui.Group, ["name"]: name + "_skill", ["parent"]:name + "_bg", ["title"]:"", ["x"]: 9, ["y"]: 28, ["w"]: 80, ["h"]: 80, ["messageFlag"]: true },
		 ...
		 { ["index_type"]: gui.RichDisplayer, ["name"]: name + "_instruct" ,["parent"]:name + "_bg", ["title"]:"", ["font"]: "ht_20_cc", ["image"]: "", ["color"]: "ublack", ["x"]: 122, ["y"]: 60, ["w"]: 240, ["h"]: 70, ["messageFlag"]: true },
		 { ["index_type"]: gui.Button, ["name"]: name + "_btnUp" ,["parent"]:name + "_bg", ["title"]:Localize_cns("ROLE_TXT35"), ["font"]: "ht_24_cc_stroke", ["image"]: "ty_tongYongBt2", ["color"]: gui.Color.white, ["x"]: 382, ["y"]: 53, ["w"]: 0, ["h"]: 0, ["event_name"]: egret.TouchEvent.TOUCH_TAP, ["fun_index"]:this.onBtnUpClick },
		];
    ...
    refreshItemWindow(window, id, level) {
	   ...
	   // if(type == "demage")  {
		...
		let desStr = String.format(SkillSystem.getInstance().getSkillDes(id))	
