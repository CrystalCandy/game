class PetPokedexWindow extends BaseCtrlWnd {
    ...
    refreshFrame() {
        ...
        for (let i in GameConfig.PetFunTipsConfig) {
            ...
            let window = this.scroll.getItemWindow(tonumber(i), w, h, 0, 0, 0)
    ...
    initItemWindow(window, row, column) {
        ...
        for (let i = 0; i < row * column; i++) {
            ...
            JsUtil.arrayInstert(Info, { ["index_type"]: gui.RichDisplayer, ["name"]: "petName_" + name + i, ["parent"]: "pet_" + name + i, ["title"]: "", ["font"]: "", ["color"]: gui.Color.white, ["x"]: 0, ["y"]: h - 35, ["w"]: w, ["h"]: 30 })
        }
