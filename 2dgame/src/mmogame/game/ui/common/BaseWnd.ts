...
class BaseWnd extends TClass {
	...
	createDotTipsUI(parentWnd:egret.DisplayObjectContainer|egret.DisplayObject , autoDelete?:boolean) {
		if (parentWnd == null) {
			return
		}
		if(autoDelete == null)
		...
		if (!dotWnd) {
			let elemInfo: any = [
				{ ["index_type"]: eui.Image, ["name"]: "btnTips", ["title"]: "", ["font"]: "ht_20_lc", ["image"]: "zjm_hongDian01", ["color"]: gui.Color.white, ["x"]: 0, ["y"]: 0, ["w"] : 40,["h"] : 40, ["event_name"]: null, ["fun_index"]: null, ["messageFlag"]: true },
			]
