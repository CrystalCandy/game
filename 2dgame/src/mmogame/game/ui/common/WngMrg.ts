...
function PopUIShow(exceptHideList?: string[]) {
...
class WngMrg extends TClass {
	...
	pushShowStatck(exceptPushList, exceptHideList) {
		...
		for (let name in copy_list) {
			...
			if (window.isVisible() && window.info.common != true && table_isExsit(exceptPushList, window.classname) == false) {
				//saveData.visibleList[name] = window
				saveData.visibleList[window.hashCode] = window.getShowOrder()
			}
			if (table_isExsit(exceptHideList, window.classname) == false) {
				window._hideWnd() //隐藏所有当前窗口
			}
		}
	}

	popShowStatck(exceptHideList) {
		...
		for (let name in copy_list) {
			...
			if (!table_isExsit(exceptHideList, window.classname)) {
				window.hideWnd()
			}

			let order = saveData.visibleList[window.hashCode]
			if (order && window.isAutoStackReshow()) {
