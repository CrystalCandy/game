class DailySanBaiWindow extends BaseCtrlWnd {
    ...
    onRefresh(){
      ...
      //  this.mElemList["group_xiangYao"].visible = true;
        ...
        let canOneKey = (had == 300)?true : false
        let state1 = true
        let state2 = false
	...
	onOneKeyClick(args){
        ...
        if(index != 2) return
        let actInfo  = GetActivity(ActivityDefine.Boss).getSanBaiInfo()
        let tempConfig = GetActivity(ActivityDefine.Boss).getSanBaiConfigByLevel(GetHeroProperty("level"))
        if(tempConfig == null){
            return
        }
        ...
        let had = GetHeroMoney(config.unity)
