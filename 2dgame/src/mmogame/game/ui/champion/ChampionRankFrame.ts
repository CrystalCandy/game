class ChampionRankFrame extends ActivityRankBaseFrame {
	...
	refreshInfo(){
		...
		let point = rewardInfo.point(rank)
		let bindCurrency = rewardInfo.bindCurrency(rank)
	...
	refreshItemWindow(window, config) {
		...
		let point = rewardInfo.point(rankNum)
		let bindCurrency = rewardInfo.bindCurrency(rankNum)
		let rdText = point+"#BIND_YUANBAO"+bindCurrency+"#JJC_POINT"
