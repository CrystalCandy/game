				...
				if (power.effect == powerEffects.EFFECT_HP_LESS) {
					...
					if (actor) {
						...
						if (emActor && emActor.isDeadState() == true) {
							...
							let [_, time] = this.mFightSystem.getCurFightTime()
							...
							let timeStr = getFormatDiffTime(Math.floor(time / 1000))

							let msgString = String.format(Localize_cns("DEMAGE_SCORSE_TIPS"), side, pos, this.result.spellId, eSide, ePos, power.point) + "  " + timeStr
