...
function GetPetLvProperty(petId) {
    ...
    let netInfo = PetSystem.getInstance().getPetInfo(petId)
...
function GetPetGrowProperty(petId) {
    let growconfig = PetSystem.getInstance().getPetGrowInfo(petId)
    let netInfo = PetSystem.getInstance().getPetInfo(petId)
...
function GetPetProperty(petId) {
    let lvpro = GetPetLvProperty(petId)
    let growpro = GetPetGrowProperty(petId)
...
//单件角色装备附加属性 -- 
//强化属性
function GetForgeCellProperty(typeName , pos) { //pos 哪一件装备
    let levelList = ForgeSystem.getInstance().getForgeInfo(typeName)
    let level = levelList[pos]
    let config = ForgeSystem.getInstance().getCellForgeConfig(typeName , pos , level)
    return config
}
