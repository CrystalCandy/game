...
var ForgeStrengthenWindow = (function (_super) {
    __extends(ForgeStrengthenWindow, _super);
    function ForgeStrengthenWindow() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    ForgeStrengthenWindow.prototype.initObj = function () {
        var params = [];
    ...
    ForgeStrengthenWindow.prototype.onLoad = function () {
    ...
    ForgeStrengthenWindow.prototype.onUnLoad = function () {
    };
    ForgeStrengthenWindow.prototype.onShow = function () {
        ...
        var str = String.format(Localize_cns("FORGE_BTN"), Localize_cns("FORGE_TXT1"));
        this.mElemList["btn_oneKey"].text = str;
        this.onInit();
        this.onRefresh();
    };
    ForgeStrengthenWindow.prototype.onHide = function () {
    ...
    ForgeStrengthenWindow.prototype.onInit = function () {
    ...
    ForgeStrengthenWindow.prototype.onRefresh = function () {
        ///刷新大师 
    ...
    ForgeStrengthenWindow.prototype.onInitForce = function () {
    ...
    ForgeStrengthenWindow.prototype.onTimer = function () {
        for (var i = 1; i <= 10; i++) {
            this.mElemList["equipItem_select" + i].visible = false;
        }
        this.select += 1;
    ...
    ForgeStrengthenWindow.prototype.onRefreshDes = function () {
        var endIndex = 11;
        var isEnd = false;
        ...
        var had = GetHeroMoney(costConfig.moneyunit);
        var maxCost = costConfig.money;
        var cStr;
        if (had >= maxCost) {
            cStr = "#lime";
        }
        ...
        var itemStr = GetMoneyIcon(costConfig.moneyunit);
    ...
    ForgeStrengthenWindow.prototype.refreshForceNum = function (force) {
        DrawNumberStringImage(this.mElemList["bImage"], "zhanLi_", "z" + force, 0, 0, -3);
    };
    //////响应事件
    ForgeStrengthenWindow.prototype.onOneKeyClick = function () {
        if (this.timer != null)
            return;
        var index = this.mParentWnd.tabWndList.getTabIndex();
        if (index == 0) {
            //let countId = GameConfig.FunForgeConfig[this.type][this.recvList[this.select-1]+1].moneyunit
            var had = GetHeroProperty("funds"); //ItemSystem.getInstance().getItemCount(countId)
            var maxCost = GameConfig.FunForgeConfig[this.type][this.recvList[this.select - 1] + 1].money;
            if (had < maxCost) {
    ...
    return ForgeStrengthenWindow;
}(BaseCtrlWnd));
__reflect(ForgeStrengthenWindow.prototype, "ForgeStrengthenWindow");
//# sourceMappingURL=ForgeStrengthenWindow.js.map