...
function UseItem(item, count, bConfirm, useTitle) {
    ...
    if (!bConfirm) {
        ...
        var name_1 = item.getName();
        var txt = String.format(Localize_cns("ITEM_USE_TIPS"), itemName);
...
//经过排序的属性列表(id)
var sortFieldKeyList = null;
function GetSortEquipFieldList() {
    if (sortFieldKeyList == null) {
        sortFieldKeyList = [];
        for (var _ in abilityNameToIndex) {
            var v = abilityNameToIndex[_];
...
function KeepFloatTwoBit(value) {
    var valueStr = "";
...
function KeepFloatTwoBitPetcent(value) {
    var valueStr = "";
    if (MathUtil.isFloat(value)) {
...
function TransferItemConfigValue(word, value) {
    var percentType = [
    ...
    var valueStr = null;
    if (table_isExsit(percentType, word)) {
        if (MathUtil.isFloat(value)) {
            valueStr = String.format("%.2f", value * 100) + "%";
        }
        else {
    ...
    else if (table_isExsit(notIntType, word)) {
        if (MathUtil.isFloat(value)) {
            valueStr = String.format("%.2f", value);
        }
    }
...
function MakeLongNumberShort(value) {
    ...
    if (type(value) == "number") {
        ...
        //显示亿
        if (value > 100000000) {
            var shortNum_1 = String.format("%.1f", value / 100000000);
            return shortNum_1 + Localize_cns("A_HUNDRED_MILLION");
        }
        //显示万
        var shortNum = String.format("%.1f", value / 10000);
...
function getMaxConfigEnhanceLevel(enhanceLevel) {
    var minEnhanceLevel = 0;
    var lastWordCountEntry = GameConfig.EquipConfigNew2["lastAttrCount"].enLevel;
    for (var configLevel_ in lastWordCountEntry.value) {
        //let _ = lastWordCountEntry.value[configLevel]
        var configLevel = tonumber(configLevel_);
...
function GetHeroMoney(unitType) {
    var money = 0;
    if (opItemUnit.FUNDS == unitType) {
        money = GetHeroProperty("funds");
    }
    else if (opItemUnit.CURRENCY == unitType) {
        money = GetHeroProperty("gold");
    }
    else if (opItemUnit.JJC_POINT == unitType) {
        ...
        var moneyTable = GetHeroProperty("moneyTable");
        if (moneyTable) {
...
function GetActorEquipPosTypeInfo() {
    var pos2Type = (_a = {},
        _a[1] = opItemType.ITEM_TYPE_WEAPON,
        _a[2] = opItemType.ITEM_TYPE_CAP,
        _a[3] = opItemType.ITEM_TYPE_CLOTH,
        _a[4] = opItemType.ITEM_TYPE_MASK,
        _a[5] = opItemType.ITEM_TYPE_NECK,
        _a[6] = opItemType.ITEM_TYPE_SHOE,
        _a);
    return pos2Type;
    var _a;
}
...
function GetActorEquipByPos(actorInfo, pos) {
    ...
    var posName = EquipPosName[pos];
    return actorInfo[posName];
}
function GetActorEquipByType(actorInfo, itemType) {
    var posTypeInfo = GetActorEquipPosTypeInfo();
    var pos = -1;
    for (var _k in posTypeInfo) {
        var k = tonumber(_k);
        var type_1 = posTypeInfo[k];
        if (type_1 == itemType) {
            pos = k;
            break;
        }
...
function ResetActorEquip(actorInfo) {
    ...
    for (var _ in EquipPosName) {
        var name_2 = EquipPosName[_];
        delete actorInfo[name_2];
    }
}
...
function AnalyPrizeFormat(prizeList) {
    var list = []; //根据奖励的填表格式返回一个列表[[entryId, count], [entryId, count], [entryId, count],]
    prizeList.forEach(function (v) {
        var t = null;
        if (v[0] == "item") {
            t = [v[1], v[2]];
        }
...
function CheckBeiBaoEquipIsFull() {
    var equipList = ItemSystem.getInstance().getEquipItemList();
    var maxCapacity = GetHeroProperty("equipMax");
    if (size_t(equipList) >= maxCapacity) {
        var t = {
            onDialogCallback: function (result, userData) {
                if (result == true) {
...
function CheckBeiBaoEquipWillFull() {
    var equipList = ItemSystem.getInstance().getEquipItemList();
    var maxCapacity = GetHeroProperty("equipMax");
    if (size_t(equipList) >= maxCapacity - 20) {
        var t = {
            onDialogCallback: function (result, userData) {
                if (result == true) {
                    ExecuteMainFrameFunction("ronglian");
...
var _a;
//# sourceMappingURL=Interface_Item.js.map