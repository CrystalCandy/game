...
UiUtil.getCnsText = function (text) {
    ...
    if (text.indexOf("$") != -1) {
        ...
        text = StringUtil.stringReplaceWithReg(text, this._CnsReg, function (word) {
            var key = word.substring(2, word.length - 1); //字符串方式是:${hello}
...
function Localize_net_i(key) {
    var netMsg = GameConfig.NetMsgConfig[tostring(key)];
    if (netMsg == null) {
...
function Localize_net_s(key) {
    var netMsg = GameConfig.NetMsgConfig[key];
    if (netMsg == null) {
...
function Localize_info_cns(key) {
    var cnsData = GameConfig.CnsConfig[key];
    if (cnsData == null) {
        return _a = {}, _a['msg'] = "Can't found " + key + " from cns.csv!!!!ZZZzzzzzz.....", _a;
    }
    else {
        return cnsData;
    }
...
//# sourceMappingURL=GameCns.js.map