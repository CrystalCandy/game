...
function commonCmp(param, value) {
    ...
    if (param[0] == ">") {
        ...
        if (value > param[1] && value < param[2]) {
            return true;
        }
    }
    else if (param[0] == "==") {
...
(function (TaskOpSpace) {
    ...
    function isMarkNode(param) {
        ...
        return table_isExsit(taskInfo.data[taskField.FIELD_CHECK_TASK_NODE] || {}, tonumber(nodeId));
    }
    function checkPetBreakLevel(param) {
        if (param["entryID"]) {
            ...
            var petList = PetSystem.getInstance().getPetInfoList();
            if (petList) {
                for (var i in petList) {
                    var petInfo = petList[i];
                    var ret = commonCmp(param, PetSystem.getInstance().getPetInfoEntry(petInfo.entry).breakThroughLevel);
                    if (ret == true) {
                        return ret;
                    }
                }
    ...
    function anyItemCount(param) {
        var t = 0;
        var itemList = param[0];
        var cmp = param[1];
        for (var entry in itemList) {
            var itemCount = itemList[entry];
            var logicItem = ItemSystem.getInstance().getItemLogicInfoByEntry(entry, storeOptions.PACKET);
    ...
    function checkFinishTask(param) {
        var taskId = param;
        var ret = TaskSystem.getInstance().isTaskHasFinished(taskId);
        if (ret == false) {
    ...
    function checkItemCount(param) {
        var entry = param[0];
        var cmpParam = param[1];
        var itemList = ItemSystem.getInstance().getItemLogicInfoListByEntry(entry, storeOptions.PACKET);
        var count = 0;
    ...
    function findNpc(param) {
        var task_ = TaskSystem.getInstance().getTask(param);
        if (task_ == null) {
            return false;
        }
        var task = task_.getPropertyInfo();
        var finishList = task.finish[taskField.FIELD_FINISH_FINDNPC];
        var count = 0;
        if (task && task.data && task.data[taskField.FIELD_FINISH_FINDNPC]) {
            //TLog.Debug(task.data[taskField.FIELD_FINISH_FINDNPC])
            for (var k in task.data[taskField.FIELD_FINISH_FINDNPC]) {
                var v = task.data[taskField.FIELD_FINISH_FINDNPC][k];
                for (var s in finishList) {
                    var t = finishList[s];
                    if (v == t) {
                        count = count + 1;
                    }
        ...
        if (task.finish[taskField.FIELD_FINISH_FINDNPC_ONLYONE] && count > 0) {
            return true;
        } //如果是找到其中一个NPC即可的，就成功了
        if (count >= finishList.length) {
            return true;
        }
        return false;
    }
    ...
    function checkSaveRecord(param) {
        var record = getSaveRecord(param[0]);
        var flag = false;
        if (param[1] == "NULL") {
            flag = record == null;
        }
    ...
    function checkCurMapId(param) {
        var mapId = tonumber(GetStringSplitBySchool(param));
        return MapSystem.getInstance().getMapId() == mapId;
    }
    TaskOpSpace.TaskOpHandler = (_a = {},
        //客户端判断
        //[TaskOpDefine.FIELD_CHECK_PET_INFO] : checkPetInfo,					//检查部下信息
        _a[TaskOpDefine.FIELD_CHECK_LEVEL] = checkHeroLevel,
        _a[TaskOpDefine.FIELD_CHECK_TASK_NODE] = isMarkNode,
        _a[TaskOpDefine.FIELD_CHECK_PET_BREAK_LEVEL] = checkPetBreakLevel,
        _a[TaskOpDefine.FIELD_CHECK_ANY_ITEM_COUNT] = anyItemCount,
        _a[TaskOpDefine.FIELD_CHECK_FINISHTASK] = checkFinishTask,
        _a[TaskOpDefine.FIELD_CHECK_ITEM_COUNT] = checkItemCount,
        _a[TaskOpDefine.FIELD_CHECK_FINISH_NPCTASK] = findNpc,
        _a[TaskOpDefine.FIELD_CHECK_FINISHCARD] = checkFinishCard,
        _a[TaskOpDefine.FIELD_CHECK_SAVERECORD] = checkSaveRecord,
        _a[TaskOpDefine.FIELD_CHECK_CURRENT_MAP] = checkCurMapId,
        _a);
    ////////////////////////////////////////////////////////////////////////////////////
    //负数KEY是函数的相反作用
    var TempHandleBuffer = {};
    var _loop_1 = function (key) {
        var func = TaskOpSpace.TaskOpHandler[key];
        TempHandleBuffer[-key] = function (param) {
            return !func(param);
        };
    };
    for (var key in TaskOpSpace.TaskOpHandler) {
        _loop_1(key);
    }
    for (var key in TempHandleBuffer) {
        var func = TempHandleBuffer[key];
        TaskOpSpace.TaskOpHandler[key] = func;
    }
    TempHandleBuffer = null;
    var _a;
    ////////////////////////////////////////////////////////////////////////////////////
})(TaskOpSpace || (TaskOpSpace = {}));
...
// 		let taskId = param[i]
// 		let flag = false
// 		if (!table_isExsit(finishList, taskId)) {
...
// // 						if(! table_isExsit(finishTaskList, taskId) ){						//不存在，就是没有做过这个任务
// // 							hasFinish = false
// // 							break
// // 						}
// // 					}
...
// }
//# sourceMappingURL=TaskChecker_OpHandler.js.map