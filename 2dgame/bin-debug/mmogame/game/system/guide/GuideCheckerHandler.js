...
var GuideCheckSpace;
(function (GuideCheckSpace) {
    GuideCheckSpace.checkEventHandler = {};
    function checkRecord(param, args) {
        var record = this.getGuideRecord(param[0]);
        TLog.Debug("checkRecord", record, param[1], param[0]);
        //io.read()
        return record == param[1];
    }
    GuideCheckSpace.checkEventHandler[GuideListenDefine.FIELD_LISTEN_EVENT_RECORD] = checkRecord;
    function clickButtonImp(param, args, eventName) {
        var targetObj = null;
        if (param.buttonName != null) {
            ...
            var infoWindow = UI_GetWindowByInfo(param.windowInfo);
            if (infoWindow) {
    ...
    function clickButton(param, args) {
        return clickButtonImp(param, args, egret.TouchEvent.TOUCH_TAP);
    }
    GuideCheckSpace.checkEventHandler[GuideListenDefine.FIELD_LISTEN_EVENT_CLICKBUTTON] = clickButton;
    //shortButton检查gui.TouchEvent.TOUCH_SHORT事件
    function shortClickButton(param, args) {
        return clickButtonImp(param, args, gui.TouchEvent.TOUCH_SHORT);
    }
    GuideCheckSpace.checkEventHandler[GuideListenDefine.FIELD_LISTEN_EVENT_SHORTCLICKBUTTON] = shortClickButton;
    ...
    GuideCheckSpace.checkEventHandler[GuideListenDefine.FIELD_LISTEN_EVENT_LISTENUISHOW] = uiShow;
    function uiCtrlShow(param, args) {
        TLog.Debug("uiCtrlShow", args.window.classname, param.windowName);
        return args.window.classname == param.windowName;
    }
    GuideCheckSpace.checkEventHandler[GuideListenDefine.FIELD_LISTEN_EVENT_LISTENUICTRLSHOW] = uiCtrlShow;
    function uiCtrlHide(param, args) {
        TLog.Debug("uiCtrlHide", args.window.classname, param.windowName);
        return args.window.classname == param.windowName;
    }
    GuideCheckSpace.checkEventHandler[GuideListenDefine.FIELD_LISTEN_EVENT_LISTENUICTRLHIDE] = uiCtrlHide;
    ...
    GuideCheckSpace.checkEventHandler[GuideListenDefine.FIELD_LISTEN_EVENT_ACCEPTTASK] = acceptTask;
    ...
    GuideCheckSpace.checkEventHandler[GuideListenDefine.FIELD_LISTEN_EVENT_LEVELUPDATE] = heroLevelUp;
    function heroLogIn(param, args) {
        return true;
    }
    GuideCheckSpace.checkEventHandler[GuideListenDefine.FIELD_LISTEN_EVENT_LOGIN] = heroLogIn;
    ...
    GuideCheckSpace.checkEventHandler[GuideListenDefine.FIELD_LISTEN_EVENT_WINDOW_CLOSE] = closeWindow;
    ...
    function activateButton(param, args) {
        return args.funcIndex == param["funcIndex"];
    }
    GuideCheckSpace.checkEventHandler[GuideListenDefine.FIELD_LISTEN_EVENT_ACTIVATE_BUTTON] = activateButton;
    ...
    function campFirstPass(param, args) {
        return args.campaignId == (param["campaignId"] || 0);
    }
    GuideCheckSpace.checkEventHandler[GuideListenDefine.FIELD_LISTEN_EVENT_CAMPAIGN_FIRST_PASS] = campFirstPass;
    function taskSucced(param, args) {
        if (param.taskId == 0) {
            return true;
        }
        return args.taskId == param.taskId;
    }
    GuideCheckSpace.checkEventHandler[GuideListenDefine.FIELD_LISTEN_EVENT_TASK_SUCCEED] = taskSucced;
...
})(GuideCheckSpace || (GuideCheckSpace = {}));
//# sourceMappingURL=GuideCheckerHandler.js.map